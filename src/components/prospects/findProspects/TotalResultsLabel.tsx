...
const TotalResultsLabel = () => {
  const { prospects, selectedProspects, prospectsEmails } =
  ...
  const total = useMemo(() => {
    return {
      ...
      website: selectedProspects.filter((prospect) => prospect.website).length,
  ...
  }, [prospects, prospectsEmails, selectedProspects]);
